...
export default function LayoutControlPanel() {
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === '3') setLayout('speaker-split');
    }
  ...
  return (
    <div className="relative z-50">
      {isOpen && (
        <div
          ref={panelRef}
          className="absolute right-0 bottom-14 w-80 space-y-4 rounded-xl border border-dashboard-bg-light bg-dashboard-bg-darkest p-4 text-white shadow-2xl backdrop-blur-md pointer-events-auto z-50 animate-slideUp"
        >
          <div className="space-y-2">
            <button
              onClick={() => setLayout('grid')}
              className={`flex w-full items-center justify-between rounded-lg px-4 py-2.5 transition-all duration-200 hover:bg-white/10 cursor-pointer ${layout === 'grid' ? 'bg-white/10 text-white' : 'text-gray-400'
                }`}
            >
            ...
            <button
              ...
              className={`flex w-full items-center justify-between rounded-lg px-4 py-2.5 transition-all duration-200 hover:bg-white/10 cursor-pointer ${layout === 'speaker-split' ? 'bg-white/10 text-purple-400' : 'text-gray-400'
                }`}
            >
              <span className="flex items-center gap-3">
          ...
          <div className="flex justify-around border-t border-gray-800/50 pt-3">
            <button
              ...
              className={`flex items-center gap-2 rounded-lg px-4 py-2 transition-all duration-200 cursor-pointer ${fitMode === 'fill' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 rounded-lg px-4 py-2 transition-all duration-200 cursor-pointer ${fitMode === 'fit' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'
                }`}
            >
